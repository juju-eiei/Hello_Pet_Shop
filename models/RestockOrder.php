...
class RestockOrder {
    ...
    public function getAll($keyword = '') {
        $query = "SELECT r.*, CONCAT(e.first_name, ' ', e.last_name) as employee_name,
                  (SELECT COUNT(*) FROM restock_details rd WHERE rd.restock_id = r.restock_id) as total_items
                  FROM restock_orders r
                  LEFT JOIN employees e ON r.employee_id = e.employee_id";
        
        if (!empty($keyword)) {
            $query .= " WHERE r.supplier_name LIKE :keyword OR e.first_name LIKE :keyword OR e.last_name LIKE :keyword";
        }
    ...
    public function create($data) {
        try {
            ...
            foreach ($data['items'] as $item) {
                ...
                $stmtUpdateStock->execute([$qty, $cost, $p_id]);
