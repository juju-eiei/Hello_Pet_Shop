...
const jsStart = c.indexOf('function renderCustomers(data) {');
const jsEndStr = "document.getElementById('searchInput').addEventListener";
const jsEnd = c.indexOf(jsEndStr);

const newRender = `function renderCustomers(data) {
...
c = c.substring(0, jsStart) + newRender + c.substring(jsEnd);

fs.writeFileSync('c:/laragon/www/Hello_Pet_Shop/admin_customers.html', c);
console.log('Customer UI safely updated.');
