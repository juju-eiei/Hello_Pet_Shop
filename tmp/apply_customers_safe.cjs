...
const newCSS = `
    <style>
        ...
        /* Bottom Nav Highlight Fix inherited */
...
// Safe replacement
const parts = c.split(/function renderCustomers\(data\) \{[\s\S]*?\}\n\n\s*document\.getElementById\('searchInput'\)/);

if (parts.length === 2) {
    c = parts[0] + newRender.trim() + '\\n\\n        document.getElementById(\\'searchInput\\')' + parts[1];
    fs.writeFileSync('c:/laragon/www/Hello_Pet_Shop/admin_customers.html', c);
    console.log('Customer UI safely updated via split.');
} else {
    // Fallback if split didn't match exactly because of formatting
    const oldFnStart = 'function renderCustomers(data) {';
    const oldFnEnd = "document.getElementById('searchInput').addEventListener";
    
    const startIndex = c.indexOf(oldFnStart);
    const endIndex = c.indexOf(oldFnEnd);
    
    if (startIndex !== -1 && endIndex !== -1) {
        c = c.substring(0, startIndex) + newRender.trim() + '\\n\\n        ' + c.substring(endIndex);
        fs.writeFileSync('c:/laragon/www/Hello_Pet_Shop/admin_customers.html', c);
        console.log('Customer UI safely updated via absolute index.');
    } else {
